...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="gta-copy h-[58vh] md:h-[72vh] p-6 sm:p-8 rounded-3xl ring-1 ring-white/10 bg-black/60 backdrop-blur-md flex flex-col justify-center">
      ...
      <p className="mt-3 text-white/80 leading-relaxed">{product.description}</p>
      ...
      <div className="mt-4">
        <button
          ...
          className={`w-full inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-medium ring-1 ring-white/10 transition ${product.theme.buttonClass}`}
        >
...
export default function ZolarShowcase() {
  ...
  return (
    <section ref={scopeRef} aria-label="Zolar product showcase" className="relative">
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-8 py-24 md:py-36 space-y-28 md:space-y-40">
        {products.map((product, idx) => {
          ...
          return (
            <article key={product.id} id={product.id} className="gta-item grid md:grid-cols-12 gap-6 md:gap-10 items-center">
              {/* Media */}
